/* ============================================================================
   ...
   GitHub Pages ne peut pas appeler directement l'API Kobo de production à
   cause de CORS. Toutes les requêtes passent par le Worker Cloudflare fourni.
   - Lecture CMS : Worker -> KPI API v2 /api/v2/assets/{uid}/data/
   - Création commande : Worker -> OpenRosa https://kc.kobotoolbox.org/submission
   ============================================================================ */
...
const KOBO = Object.freeze({
    ...
    // OBLIGATOIRE sur GitHub Pages : URL du Worker Cloudflare fourni dans le pack.
    // Exemple : https://boeny-kobo-proxy.votre-sous-domaine.workers.dev
...
async function startBoenyNectar() {
    try {
        if (!tokenIsConfigured()) {
            throw new Error("Le token Kobo n'est pas encore renseigné. Ouvrez script.js et renseignez TOKEN.");
        }
        if (!proxyIsConfigured()) {
            throw new Error("Le proxy CORS Cloudflare Worker n'est pas encore configuré. Déployez cloudflare-worker-kobo-proxy.js puis collez son URL dans PROXY_BASE_URL de script.js.");
        }

        showStatus("Connexion à KoboToolbox via le proxy CORS et chargement du contenu…", "info", true);
        ...
        showStatus(`❌ ${escapeHtml(error.message)}<br><small>Aucun data.json local n'est utilisé. Les données proviennent de KoboToolbox via le proxy CORS.</small>`, "danger", true);
    }
}
