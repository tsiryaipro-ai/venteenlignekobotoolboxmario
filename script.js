/* ============================================================================
   ...
   Lecture CMS : tentative directe via l'URL publique data.json fournie.
   Si le navigateur bloque cette lecture par CORS, le Worker Cloudflare reste
   disponible comme solution de secours.
   Création commande : le Worker reste requis tant que l'envoi privé Kobo
   utilise le token d'authentification depuis GitHub Pages.
   ============================================================================ */
...
const KOBO = Object.freeze({
    ...
    // Facultatif pour la LECTURE du CMS si l'URL publique ci-dessous fonctionne
    // directement depuis GitHub Pages. Toujours nécessaire pour les opérations
    // privées (ex. enregistrement des commandes) si CORS bloque l'API Kobo.
...
async function getAllCmsSubmissions() {
    // 1) Première tentative : lecture directe et anonyme de l'URL data.json
    // communiquée par l'utilisateur. Cela évite Cloudflare si Kobo autorise
    // cette origine et si le projet est publiquement lisible.
    if (KOBO.CMS_DATA_URL) {
        try {
            const direct = await fetch(KOBO.CMS_DATA_URL, {
                method: "GET",
                cache: "no-store",
            ...
            throw new Error("Format JSON Kobo inattendu.");
        } catch (directError) {
            console.warn("Lecture directe du CMS impossible :", directError);
            if (!proxyIsConfigured()) {
                throw new Error(
                    "L’URL Kobo CMS a bien été configurée, mais le navigateur ne peut pas la lire directement (projet privé ou CORS). " +
                    "Dans ce cas, configurez PROXY_BASE_URL pour utiliser le proxy uniquement comme solution de secours."
                );
            }
        }
    }

    // 2) Secours : lecture authentifiée via proxy.
...
async function attachmentToObjectUrl(submission, fileName) {
    ...
    try {
        const resp = await koboFetchRaw(absoluteKoboUrl(rawUrl));
...
async function startBoenyNectar() {
    try {
        ...
        showStatus(`❌ ${escapeHtml(error.message)}<br><small>Aucun data.json local n'est utilisé. Le CMS est lu directement depuis KoboToolbox quand cela est autorisé, sinon via le proxy configuré.</small>`, "danger", true);
    }
}
