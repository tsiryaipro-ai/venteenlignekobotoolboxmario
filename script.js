/* ============================================================================
   ...
   Lecture CMS : stratégie robuste inspirée du tableau de bord fourni :
   Direct KoboToolbox -> CorsBridge -> AllOrigins -> CORS.lol.
   Aucun token Kobo n'est transmis à ces proxys publics. Cette chaîne fonctionne
   uniquement si les soumissions CMS sont accessibles anonymement.
   Création commande : opération privée, donc un proxy/backend authentifié reste
   requis tant que Kobo bloque les appels cross-origin avec Authorization.
   ============================================================================ */
...
const KOBO = Object.freeze({
    ...
    // NON requis pour la lecture publique du CMS : la lecture utilise une chaîne
    // Direct -> CorsBridge -> AllOrigins -> CORS.lol.
    // Reste utile pour les opérations privées (notamment l'enregistrement des commandes).
    ...
    // Projet CMS communiqué le 13/09/2026.
    CMS_ASSET_UID: "aRDi7maSxCyYuAT8Pd9JR4",
    CMS_DATA_URL: "https://kf.kobotoolbox.org/api/v2/assets/aRDi7maSxCyYuAT8Pd9JR4/data.json",

    // Laissez vide pour la détection automatique du projet Commandes via le proxy.
    ORDERS_ASSET_UID: "",
...
/* ---------------------- LECTURE PUBLIQUE KOBO / CORS ---------------------- */

/*
   Stratégie inspirée du code fourni par l'utilisateur :
   1) accès direct KoboToolbox ;
   2) CorsBridge ;
   3) AllOrigins ;
   4) CORS.lol.

   IMPORTANT : aucun token Kobo n'est envoyé à ces services tiers.
   Cette chaîne est donc réservée aux données Kobo que vous rendez accessibles
   anonymement. Si le CMS est privé, utilisez le proxy/backend privé configuré
   dans PROXY_BASE_URL à la place.
*/
...
async function fetchKoboPublicWithFallback(targetUrl, { as = "json" } = {}) {
    const errors = [];
    for (const provider of KOBO_PUBLIC_PROXY_PROVIDERS) {
        const requestUrl = provider.buildUrl(targetUrl);
        try {
            console.log(`[Boeny/Kobo] Tentative CMS via ${provider.name}`);
            ...
            if (as === "response") return { response, provider: provider.name };
            let body;
            try {
            ...
            console.warn(`[Boeny/Kobo] Échec via ${provider.name}:`, error);
        }
    }
    throw new Error(`Toutes les méthodes publiques de lecture KoboToolbox ont échoué. ${errors.join(" | ")}`);
}

function normalizeKoboNextUrl(nextUrl) {
    if (!nextUrl) return null;
    try {
        return new URL(String(nextUrl), KOBO.KPI_BASE_URL).href;
    } catch (_) {
        return null;
    }
}

async function fetchAllKoboPublicPagesWithFallback(initialUrl, maxPages = 100) {
    const rows = [];
    const providers = [];
    let url = initialUrl;
    let page = 0;
    ...
    while (url && page < maxPages) {
        const result = await fetchKoboPublicWithFallback(url, { as: "json" });
        ...
        rows.push(...pageRows);
        page += 1;
        url = Array.isArray(body) ? null : normalizeKoboNextUrl(body?.next);
    }
...
function proxyIsConfigured() {
    return Boolean(KOBO.PROXY_BASE_URL && !KOBO.PROXY_BASE_URL.includes("COLLEZ_ICI"));
}

function proxyUrlFor(targetUrl) {
    if (!proxyIsConfigured()) {
        throw new Error("Le proxy CORS n'est pas encore configuré. Déployez cloudflare-worker-kobo-proxy.js puis collez son URL dans PROXY_BASE_URL de script.js.");
    }
    const base = KOBO.PROXY_BASE_URL.replace(/\/$/, "");
    return `${base}/?url=${encodeURIComponent(targetUrl)}`;
}

async function koboFetchRaw(url, options = {}) {
    let response;
    try {
        response = await fetch(proxyUrlFor(url), {
            ...options,
            headers: authHeaders(options.headers || {})
        });
    } catch (error) {
        throw new Error(`Connexion au proxy impossible : ${error.message || error}. Vérifiez PROXY_BASE_URL et le déploiement Cloudflare Worker.`);
    }
...
async function koboFetchJson(url, options = {}) {
    const response = await koboFetchRaw(url, options);
...
async function resolveProjectUids() {
    CMS_ASSET_UID_RESOLVED = KOBO.CMS_ASSET_UID || null;

    // La détection du projet Commandes nécessite l'API authentifiée.
    // On ne la fait au démarrage que si le proxy est configuré.
    if (proxyIsConfigured()) {
...
async function getAllCmsSubmissions() {
    const publicUrl = KOBO.CMS_DATA_URL || `${KOBO.KPI_BASE_URL}/api/v2/assets/${encodeURIComponent(CMS_ASSET_UID_RESOLVED)}/data.json`;

    // 1) Chaîne publique robuste : Direct -> CorsBridge -> AllOrigins -> CORS.lol.
    // Aucun token n'est transmis aux services publics.
    try {
        const result = await fetchAllKoboPublicPagesWithFallback(`${publicUrl}${publicUrl.includes("?") ? "&" : "?"}limit=1000&_t=${Date.now()}`);
        CMS_LAST_PROVIDERS = result.providers || [];
        if (!result.rows.length) {
            throw new Error("KoboToolbox a répondu, mais aucune soumission CMS n'a été trouvée.");
        }
        return result.rows;
    } catch (publicError) {
        console.warn("Lecture publique du CMS impossible :", publicError);

        // 2) Secours privé optionnel : proxy/backend authentifié.
        if (!proxyIsConfigured()) {
            throw new Error(
                `${publicError.message || publicError} ` +
                "La stratégie Direct/CorsBridge/AllOrigins/CORS.lol ne peut lire que des données CMS accessibles anonymement. " +
                "Rendez les soumissions CMS publiques en lecture ou configurez PROXY_BASE_URL pour un accès privé authentifié."
            );
        }
    }
    ...
    const rows = [];
    CMS_LAST_PROVIDERS = ["Proxy privé configuré"];
    while (url) {
        const page = await koboFetchJson(url);
...
function selectPublishedCmsSubmission(rows) {
    ...
    const published = rows.filter(row => String(valueOf(row, "cms_info/publication_status", "meta/publication_status", "publication_status") || "").toLowerCase() === "published");
...
async function attachmentToObjectUrl(submission, fileName) {
    ...
    try {
        let resp;
        try {
            const publicResult = await fetchKoboPublicWithFallback(absoluteKoboUrl(rawUrl), { as: "response" });
            resp = publicResult.response;
        } catch (publicError) {
            if (!proxyIsConfigured()) throw publicError;
            resp = await koboFetchRaw(absoluteKoboUrl(rawUrl));
        }
...
async function transformCmsSubmission(s) {
    ...
    return {
        brandName: valueOf(s, "cms_info/brand_name", "meta/brand_name", "brand_name") || "Boeny Nectar",
        cmsVersion: valueOf(s, "cms_info/cms_version", "meta/cms_version", "cms_version") || "",
        navigation: {
...
async function submitOrderToKobo(infos, platform) {
    if (!proxyIsConfigured()) {
        throw new Error("L'affichage public du CMS fonctionne sans Cloudflare, mais l'enregistrement privé des commandes nécessite encore un proxy/backend authentifié dans PROXY_BASE_URL.");
    }
    ...
    const response = await koboFetchRaw(KOBO.OPENROSA_SUBMISSION_URL, {
        method: "POST",
...
async function startBoenyNectar() {
    try {
        showStatus("Chargement du contenu depuis KoboToolbox…", "info", true);
        ...
        const providerText = CMS_LAST_PROVIDERS.length ? ` — accès : ${escapeHtml(CMS_LAST_PROVIDERS.join(" + "))}` : "";
        showStatus(`✅ Site chargé depuis KoboToolbox${data.cmsVersion ? ` — configuration ${escapeHtml(data.cmsVersion)}` : ""}${providerText}.`, "success", false);
    } catch (error) {
        console.error("Initialisation KoboToolbox :", error);
        showStatus(`❌ ${escapeHtml(error.message)}<br><small>Aucun data.json local n'est utilisé. Lecture essayée dans l'ordre : Kobo direct → CorsBridge → AllOrigins → CORS.lol, puis proxy privé seulement s'il est configuré.</small>`, "danger", true);
    }
}
